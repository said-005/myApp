...
return new class extends Migration
{
    ...
    {
        Schema::create('emmanchements', function (Blueprint $table) {
       $table->string('code_Emmanchement')->primary();
           $table->string('ref_production');
            $table->foreign('ref_production')->references('production_code')->on('productions')->restrictOnDelete();
            $table->date('date_Emmanchement');
    ...
    {
        Schema::dropIfExists('emmanchements');
    }
};

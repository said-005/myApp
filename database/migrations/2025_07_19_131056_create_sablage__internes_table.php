...
return new class extends Migration
{
    ...
    {
        Schema::create('sablage_internes', function (Blueprint $table) {
          $table->string('code_Sablage_Interne')->primary();
         $table->string('ref_production');
            $table->foreign('ref_production')->references('production_code')->on('productions')->restrictOnDelete();
            $table->date('date_Sablage_Interne');
    ...
    {
        Schema::dropIfExists('sablage_internes');
    }
};

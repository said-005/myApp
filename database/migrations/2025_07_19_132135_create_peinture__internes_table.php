...
return new class extends Migration
{
    ...
    {
        Schema::create('peinture_internes', function (Blueprint $table) {
            $table->string('code_Peinture_internes')->primary();
             $table->string('ref_production');
            $table->foreign('ref_production')->references('production_code')->on('productions')->restrictOnDelete();
            $table->date('date_Peinture_Interne');
    ...
    {
        Schema::dropIfExists('peinture_internes');
    }
};

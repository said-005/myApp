...
return new class extends Migration
{
    ...
    {
        Schema::create('peinture_externes', function (Blueprint $table) {
          $table->string('code_Peinture_Externe')->primary();
 $table->string('ref_production');
            $table->foreign('ref_production')->references('production_code')->on('productions')->restrictOnDelete();
            $table->date('date_Peinture_Externe');
    ...
    {
        Schema::dropIfExists('peinture_externes');
    }
};
